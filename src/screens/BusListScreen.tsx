    ...
    const renderItem = ({item: bus}: {item: IBus}) => {
        return (
            <TouchableOpacity onPress={() => navigate(screens.seatSelectionScreen, {busExternalId: bus.busExternalId})} className={'mb-4 p-4 rounded-lg shadow-sm bg-white'}>
                ...
                <Text className={'text-lg text-gray-900 font-okra font-bold'}>{bus.company}</Text>
                <Text className={'text-sm text-gray-500'}>{bus.busType}</Text>
                ...
                <View className={'flex-row justify-between items-center mt-2'}>
                    <Text className={'text-base text-green-600 font-okra font-semibold'}>₹{bus.price}</Text>
                    ...
                    {/*<Text>{bus.seats.flat().filter(seat => !seat.isBooked).length} Seats</Text>*/}
                </View>
            </TouchableOpacity>
        );
    };
