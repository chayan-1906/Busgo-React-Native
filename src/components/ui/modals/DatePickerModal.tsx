...
function DatePickerModal({isVisible, onClose, onConfirm, selectedDate}: DatePickerModalProps) {
    ...
    if (Platform.OS === 'android') {
        return <DateTimePicker value={tempDate} mode={'date'} display={'compact'} minimumDate={today} maximumDate={twoMonthsAfter} onChange={onChangeAndroid} />;
    }
        ...
        <Modal transparent={true} visible={isVisible} animationType={'slide'}>
            <View className={'flex-1 justify-center'} style={{backgroundColor: 'rgba(0,0,0,0.5)'}}>
                <View className={'bg-white p-4 rounded-3xl mx-2 items-center'}>
                    {Platform.OS === 'ios' && <DateTimePicker value={tempDate} mode={'date'} display={'spinner'} minimumDate={today} maximumDate={twoMonthsAfter} onChange={(event: DateTimePickerEvent, date: Date | undefined) => event.type === 'set' && date && setTempDate(date)} />}
