...
import CustomViewShot from "@/components/ui/CustomViewShot.tsx";
			...
			<View className={'flex-1 justify-center items-center'} style={{backgroundColor: 'rgba(42, 37, 38, 0.9)'}}>
				...
				{bookingInfo && bookingInfo.bus ? (
					<View className={'bg-white overflow-hidden rounded-xl w-[90%] shadow-lg relative'}>
						<CustomViewShot ref={viewShotRef} style={{backgroundColor: 'white', padding: 16}}>
							<View className={''}>
								...
								<View className={'p-3 rounded-lg bg-gray-200 border-l-4 border-tertiary'}
								      style={{shadowOffset: {width: 2, height: 2}, shadowOpacity: 0.4, shadowRadius: 5, elevation: 6}}>
									<Text className={'text-lg font-okra-bold text-gray-900'}>{bookingInfo.bus?.from} → {bookingInfo.bus?.to}</Text>
								...
								<View className={'flex-row mt-6 gap-4'}>
									{/** about bus */}
									<View className={'p-3 rounded-lg bg-gray-200 border-l-4 border-tertiary flex-1'}
									      style={{shadowOffset: {width: 2, height: 2}, shadowOpacity: 0.4, shadowRadius: 5, elevation: 6}}>
							...
							{/** curves - svgs */}
							<View className={`absolute left-[-14px] ${Platform.OS === 'android' ? 'top-[69%]' : 'top-[66%]'} -translate-y-1/2`}>
								<Svg height={40} width={28}>
									<Circle cx={14} cy={20} r={14} fill={'#2A2526'}/>
								</Svg>
							</View>
							<View className={`absolute right-[-14px] ${Platform.OS === 'android' ? 'top-[69%]' : 'top-[66%]'} -translate-y-1/2`}>
								<Svg height={40} width={28}>
									<Circle cx={14} cy={20} r={14} fill={'#2A2526'}/>
								</Svg>
							</View>
